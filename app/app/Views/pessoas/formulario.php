...
<div class="container mx-auto max-w-xl px-4 py-8">
    ...
    // Se houver erros de validação, os campos old() serão usados automaticamente, mas
    // este array $dados garante que os valores do $pessoa sejam carregados na edição.
    $dados = $is_edit ? $pessoa : [];
    ?>

    <h1 class="text-3xl font-bold text-gray-800 mb-6 border-b pb-2"><?= esc($title) ?></h1>

    <!-- Mensagem de Erro de Validação Global (caso o formulário não passe na validação) -->
    ...
    <form action="<?= $action ?>" method="post" class="space-y-6 bg-white p-6 rounded-xl shadow-lg">
        ...
        <!-- Token CSRF (Prevenção de Falsificação de Solicitação entre Sites) -->
        <?= csrf_field() ?>
        ...
        <div>
            ...
            <input type="text" id="cpf_cnpj" name="cpf_cnpj"
                ...
                placeholder="Apenas números (Ex: 12345678901 ou 00000000000000)"
                required>
